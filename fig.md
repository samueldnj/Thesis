...
<!-- $\alpha_q$ & Shape parameter for multispecies prior on species specific prior variance for $\log$ catchability $\kappa_s^2$  \\
$\beta_q$ & Scale parameter for multispecies prior on species specific prior variance for $\log$ catchability $\kappa_s^2$  \\ -->
...
\begin{table}[htbp]
\begin{center}
\caption{A truncated version of the hierarchical stock assessment model used as the estimation procedure in Chapter 3. The hierarchical model in this version shares a prior distribution on process error deviations $\zeta_{s,t}$, representing correlations in environmental variation due to species with shared habitat. All variables used here are defined in Table \ref{tab:OMspec}.}\label{tab:Ch2est}
\begin{tabular}{cl}
\hline
& \textbf{Assessment Input Parameters} \\
